...
if [ $? -ne 0 ]; then
    echo "UNHEALTHY: Cannot reach SMA daemon at $URL"
    exit 1
fi

status=$(echo "$response" | python3 -c "import sys,json; print(json.load(sys.stdin).get('status',''))" 2>/dev/null)
if [ "$status" = "ok" ]; then
    echo "HEALTHY: SMA daemon responding at $URL"
    exit 0
else
    echo "UNHEALTHY: SMA daemon returned status '$status'"
    exit 1
fi
