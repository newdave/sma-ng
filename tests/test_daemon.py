...
class TestAdminHTML:
  """Test ADMIN_HTML template structure and content."""

  def test_valid_html_structure(self):
    assert "<!DOCTYPE html>" in ADMIN_HTML
    assert "<html" in ADMIN_HTML
    assert "<head>" in ADMIN_HTML
    assert "<body" in ADMIN_HTML
    assert "</html>" in ADMIN_HTML

  def test_admin_endpoints_referenced(self):
...
class TestDocsTemplate:
  ...
  def test_marks_index_as_active(self):
    html = _load_docs_template("index")
    assert 'href="/docs" class="bg-gray-700 text-white"' in html

  def test_marks_selected_slug_as_active(self):
    html = _load_docs_template("daemon")
    assert 'href="/docs/daemon" class="bg-gray-700 text-white"' in html
    assert 'href="/docs" class="text-gray-300 hover:text-white"' in html
...
class TestNodeHostResolution:
  ...
  def test_is_public_ip_address_rejects_loopback_and_unspecified(self):
    assert _is_public_ip_address("127.0.0.1") is False
    assert _is_public_ip_address("0.0.0.0") is False

  def test_is_public_ip_address_rejects_invalid_values(self):
    assert _is_public_ip_address("not-an-ip") is False

  def test_is_public_ip_address_accepts_non_loopback_address(self):
    assert _is_public_ip_address("10.0.0.20") is True
  ...
  def test_resolve_advertised_host_uses_outbound_interface_when_hostname_is_loopback(self):
    with patch("resources.daemon.server.socket.socket") as mock_sock:
      socket_ctx = mock_sock.return_value.__enter__.return_value
