...
from daemon import (
    JobDatabase, PathConfigManager, ConfigLogManager, ConfigLockManager,
    DaemonServer, WebhookHandler,
    DASHBOARD_HTML,
        ...
        try:
            with job_db._cursor() as cursor:
                cursor.execute(
                    ...
                    ('/tmp/x.mkv', '/cfg.ini', '[]', STATUS_PENDING)
                )
...
class TestHTTPEndpoints:
    ...
    def test_webhook_json_string_arg(self, live_server, tmp_path):
        ...
        f = tmp_path / 'movie.mkv'
        f.touch()
        data, status = self._post(live_server, '/webhook',
                                  data={'path': str(f), 'args': '-tmdb 603'})
        assert status == 202
