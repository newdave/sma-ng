...
class TestSMAConfigParserGetDict:
  ...
  def test_invalid_value_modifier_is_skipped_and_logged(self):
    p = self._make_parser("test", "mapping", "hevc:not-a-float")
    p.log = MagicMock()
    result = p.getdict("test", "mapping", valueModifier=float)
    assert result == {}
    p.log.exception.assert_called_once()
...
class TestSMAConfigParserGetPath:
  ...
  def test_create_ignores_makedirs_failure(self, mock_makedirs):
    p = self._make_parser("test", "path", "/some/path")
    result = p.getpath("test", "path", create=True)
    assert result == os.path.normpath("/some/path")
...
class TestSMAConfigParserGetDirectories:
  def test_creates_each_directory(self, tmp_path):
    p = SMAConfigParser()
    p.add_section("test")
    first = tmp_path / "one"
    second = tmp_path / "two"
    p.set("test", "dirs", f"{first},{second}")
    result = p.getdirectories("test", "dirs")
    assert os.path.normpath(str(first)) in result
    assert os.path.normpath(str(second)) in result
    assert first.is_dir()
    assert second.is_dir()

  @patch("os.makedirs", side_effect=OSError("denied"))
  @patch("os.path.isdir", return_value=False)
  def test_ignores_directory_creation_failures(self, mock_isdir, mock_makedirs, tmp_path):
    p = SMAConfigParser()
    p.add_section("test")
    first = tmp_path / "one"
    second = tmp_path / "two"
    p.set("test", "dirs", f"{first},{second}")
    result = p.getdirectories("test", "dirs")
    assert result == [os.path.normpath(str(first)), os.path.normpath(str(second))]
