...
def _make_settings(username='', servername='', host='', port=32400,
                   token='', password='', ssl=False, ignore_certs=False,
                   path_mapping=None):
    settings = MagicMock()
    settings.Plex = {
        'username': username,
...
class TestGetPlexServer:
    ...
    def test_creates_ssl_session_when_ignore_certs(self):
        ...
        with patch('autoprocess.plex.requests.Session') as mock_session_cls, \
             patch('autoprocess.plex.requests.packages') as mock_packages, \
             patch('autoprocess.plex.PlexServer') as mock_ps:
    ...
    def test_direct_server_connection_with_token(self):
        ...
        with patch('autoprocess.plex.PlexServer') as mock_ps:
            ...
            mock_ps.assert_called_once_with(
                'http://plex.local:32400', 'mytoken', session=None
            )
            assert result is mock_plex
    ...
    def test_plex_tv_token_auth(self):
        from autoprocess.plex import getPlexServer
        settings = _make_settings(
            username='user@example.com', servername='MyServer', token='mytoken'
        )
        with patch('autoprocess.plex.MyPlexAccount') as mock_account_cls:
            ...
            mock_account_cls.assert_called_once_with(
                username='user@example.com', token='mytoken', session=None
            )
            assert result is mock_plex
    ...
    def test_plex_tv_token_fails_falls_back_to_password(self):
        from autoprocess.plex import getPlexServer
        settings = _make_settings(
            username='user@example.com', servername='MyServer',
            token='badtoken', password='mypass'
        )
        call_count = {'n': 0}
