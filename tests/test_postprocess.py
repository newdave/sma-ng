...
class TestPostProcessorSetTVMultiEpisode:
    ...
    def test_gather_scripts_includes_valid_script(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            'resources.postprocess.os.path.dirname',
            lambda p: str(tmp_path / 'resources')
        )
        import resources.postprocess as pp_mod
        pp = PostProcessor.__new__(PostProcessor)
        # Call gather_scripts with the patched directory
        with patch('resources.postprocess.os.listdir', return_value=['valid.py', 'skip.pyc', 'subdir']), \
