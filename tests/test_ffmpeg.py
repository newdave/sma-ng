...
from converter.ffmpeg import FFMpeg, MediaFormatInfo, MediaInfo, MediaStreamInfo
...
[/FORMAT]"""
  ...
  def test_attachment_property(self, make_stream):
    info = MediaInfo()
    info.streams.append(make_stream(type="attachment", codec="ttf", index=5, metadata={"filename": "font.ttf", "mimetype": "font/ttf"}))
  ...
  def test_json_property(self, make_stream):
    ...
    v = make_stream(type="video", codec="h264", index=0, video_width=1920, video_height=1080, fps=24.0, pix_fmt="yuv420p", profile="main", field_order="progressive", video_level=4.1)
    v.framedata = {}
    info.streams.append(v)
    a = make_stream(type="audio", codec="aac", index=1, audio_channels=2, audio_samplerate=48000)
