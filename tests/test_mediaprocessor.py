...
class TestCoerceSdrOutput:
  ...
  def test_main10_profile_coerce_also_pins_vpix_fmt_when_none(self):
    # Regression: 10-bit SDR source with no settings.pix_fmt left vpix_fmt=None.
    # _coerce_sdr_output coerced main10 -> main but left vpix_fmt None, so
    # vpp_qsv inferred format=p010le from the source and the encoder rejected
    # P010 surfaces with "Invalid FrameType:0".
    mp = _make_mp()
    vpix, vprof, _, depth, coerced = mp._coerce_sdr_output(None, "main10", "hevc_qsv", ["hevc_qsv"], 10, self.PIX_FMTS)
    assert vpix == "yuv420p"
    assert vprof == "main"
    assert depth == 8
    assert coerced is True

  def test_source_pix_fmt_10bit_with_none_vpix_pinned_to_8bit(self):
    # Same bug shape but reached via a 10-bit source pix_fmt rather than a
    # 10-bit profile (e.g. when source profile is already 8-bit-style but
    # the actual surfaces are 10-bit).
    mp = _make_mp()
    vpix, _, _, depth, coerced = mp._coerce_sdr_output(None, "main", "hevc_qsv", ["hevc_qsv"], 10, self.PIX_FMTS, source_pix_fmt="yuv420p10le")
    assert vpix == "yuv420p"
    assert depth == 8
    assert coerced is True

  def test_source_pix_fmt_8bit_with_none_vpix_stays_none(self):
    # 8-bit source + no vpix_fmt opinion: nothing to coerce, leave vpix None
    # so downstream falls back to encoder defaults.
    mp = _make_mp()
    vpix, _, _, _, coerced = mp._coerce_sdr_output(None, "main", "hevc_qsv", ["hevc_qsv"], 8, self.PIX_FMTS, source_pix_fmt="yuv420p")
    assert vpix is None
