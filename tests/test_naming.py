...
from resources.metadata import MediaType, update_plexmatch
...
class TestNamingDataFromArrApi:
  ...
  def test_sonarr_success(self, mock_requests):
    ...
    mock_requests.get.return_value = mock_resp
    data = NamingData()
    result = data._from_arr_api({"host": "localhost", "port": 8989, "apikey": "key"}, "/tv/show.mkv", "sonarr", MagicMock())
  ...
  def test_radarr_success(self, mock_requests):
    mock_resp = MagicMock()
    mock_resp.json.return_value = {"movie": {"title": "Movie Title", "year": 2020}, "quality": {"quality": {"resolution": "4K", "source": "BluRay"}}, "releaseGroup": "FGT"}
    mock_requests.get.return_value = mock_resp
    data = NamingData()
    result = data._from_arr_api({"host": "localhost", "port": 7878, "apikey": "key"}, "/movies/film.mkv", "radarr", MagicMock())
    assert result is True
...
class TestMultiEpisodeNaming:
  ...
  def test_multi_episode_full_template(self):
    data = self._tv_data(episodes=[1, 2], episode=1, episode_title="Pilot / The Cat's in the Bag...", episode_cleantitle="Pilot  The Cat's in the Bag...", release_group="LOL")
    result = apply_template(DEFAULT_TV_TEMPLATE, data)
