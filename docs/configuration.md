...
| `force-reencode`          | bool    | `false`           | Re-encode the video stream instead of copying it, even when the codec is accepted (rebuilds GOP/keyframes); implies `process-same-extensions`                                                                                                                                                                                                                                                                                                       |
| `post-process`            | bool    | `false`           | Run post-process scripts                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `wait-post-process`       | bool    | `false`           | Wait for post-process scripts to finish                                                                                                                                                                                                                                                                                                                                                                                                             |
| `fallback-policy`         | enum    | `aggressive`      | Recovery ladder when hardware transcoding fails. `aggressive` (default) walks `hw → hw_alt → sw_decode → full_sw`, so a HW failure always falls back through software decode *and* software encode before the job is surfaced as failed. `sw_decode_only` stops before the SW encoder swap; `hw_alt` stops before any SW step; `hw_only` surfaces the original FFmpeg error immediately with no retries. **Decode-side carve-out:** under `hw_alt` and `hw_only`, a failure classified `decoder_init_failed` (the hardware decoder can't handle the source codec — common with MPEG-4 ASP / XviD on Intel QSV) still earns one software-decode + hardware-encode retry, because that keeps the expensive encode on the GPU without violating the "no software encode" intent. The deprecated boolean `software-fallback` is auto-migrated (`true → aggressive`, `false → hw_only`). |
