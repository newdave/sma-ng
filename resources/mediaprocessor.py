        ...
        if len(same_channel_options) > 1:
          ...
          if any(x for x in same_channel_options if x["codec"] == "copy" and self.getSourceStream(x["map"], info).codec in allowed_codecs):
            ...
            for codec in set(codecs):
              same_codec_options = [
                x for x in same_channel_options if Converter.codec_name_to_ffprobe_codec_name(x["codec"]) == codec or (x["codec"] == "copy" and self.getSourceStream(x["map"], info).codec == codec)
              ]
  ...
  def canBypassConvert(self, inputfile, info, options=None):
    ...
    if self.settings.output_extension == self.parseFile(inputfile)[2]:
      if not self.settings.force_convert and not self.settings.process_same_extensions:
        ...
        self.log.info(
          "Input and output extensions match and the file appears to have already been processed by SMA-NG, enable force-convert to override [force-convert: %s]." % self.settings.force_convert
        )
        return True
      elif (
